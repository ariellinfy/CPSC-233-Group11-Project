...
public class Board {
	...
	/**
	 * Default constructor that takes in no parameters, and initializes the instance
	 * variable "board".
	 */
	public Board() {
	...
	/**
	 * Constructor that takes in "boardSize" as a parameter, and initializes the
	 * instance variables "board" and "boardSize".
	 * 
	 * @param boardSize the size of the game board.
	 */
	public Board(int boardSize) {
		/*
		 * If condition checks that "boardSize" is equal to one of four board size
		 * options offered before updating the instance variable "boardSize".
		 */
	...
	/**
	 * Method that is used to initialize the game board, as well as the instance
	 * variable "alphabetList" (containing the index for the game board).
	 */
	public void initBoard() {
		for (int i = 0; i < boardSize; i++) {
			...
			/*
			 * "alphabetList" forms the index (coordinate system) to be used for the game
			 * board. The put method is used to insert a integer and character tuple each
			 * time the outer for loop is executed. Thus, the length of "alphabetList" will
			 * always be equal to "boardSize", and will always go in ascending order
			 * beginning from (1, 'A'). The key/value is incremented by one in each for loop
			 * execution, so (2, 'B') would be put into "alphabetList" following (1, 'A'),
			 * etc.
			 */
	...
	public void printBoard() {
		...
		System.out.println();
		/*
		 * Nested for loop iterates through "board" and prints out each element,
		 * creating the game board in the terminal.
		 */
		for (int r = 0; r < boardSize; r++) {
			for (int c = 0; c < boardSize; c++) {
				/*
				 * At the beginning of each new line (i.e. when "c" is equal to zero), the index
				 * for the y-axis is printed first.
				 */
				if (c == 0) {
					/*
					 * If "r" is less than 9, we must add additional whitespace when printing the
					 * index to avoid the game board becoming staggered.
					 */
					if (r < 9) {
	 ...
	 * 
	 * @return alphabetList a HashMap object containing the index of the current
	 *         game board.
	 */
