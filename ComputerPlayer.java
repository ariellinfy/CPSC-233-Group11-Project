...
public class ComputerPlayer extends Player {
	...
	/**
	 * Constructor with only difficulty level defined. Set the computer
	 * player default name to be "AI".
	 * @param difficultyLevel the difficulty level of the computer moves
	 * chosen by the user in this game (ie. how smart the computer moves).
	 */
	...
	/**
	 * Constructor with both stone color and difficulty known. Set the 
	 * computer player default name to be "AI".
	...
	/**
	 * A helper method that will be called when a ComputerPlayer object
	 ...
	 * calculateScores helper methods to generate a list of possible moves
	 * ranked in descending order and randomly picks a move from the list.
	 * @param currentConfig current game configuration used to access
	 * current board data.
	...
	public Move getMove(GameConfiguration currentConfig) {
		...
		/*
		 * Get a move list that includes a list of coord(s) that has the 
		 * highest scores from the score table.
		 * The number of coords in the list is determined by the difficulty
	...
	private ArrayList<int[]> generateMoveList(Stone[][] board, int boardSize) {
		...
		// Number of times the for loop will run is determined by the level of difficulty.
		for (int counter = 0; counter < levelMap.get(difficultyLevel); counter++) {
			...
			/*
			 * Set the current highest score to be -1, so next loop won't repeat the
			 * same index position.
			 */
	 ...
	 * for each five connected coordinates (so-called a five-tuple) in the board, we will first 
	 * determine its number of black stones and number of white stones, and use the helper method 
	 * tupleScore to give this tuple a score. This score will be added to each of these five coordinates.
	 * Repeat this process until all tuples in the board populate a score and add to the coordinates, 
	 * hence the score of each coordinate is the sum of the scores of all five-tuples that contain 
	 * this coordinate. The coordinate with the highest score is the best next move of the current game.
	 * @param board current board of the game.
	 * @param boardSize the size of the board.
